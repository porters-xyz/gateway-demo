...
The gateway primarily relies on the `net/http` library and its `ReverseProxy` functionality.
Requests are handled by `GorillaMux`, mapping paths to the reverse proxy.

## Reverse Proxy and Middleware

Inside the reverse proxy, middleware runs and hosts logic for allowing or denying requests.
Middleware plugins are injected, implementing rules for proxying or rejecting requests.
...
The database package contains logic for interactions between Redis and PostgreSQL, handling data storage and retrieval.
